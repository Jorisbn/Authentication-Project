...
export async function POST(request: Request) {
    ...
    try {
        ...
        if (!user) return Response.json({ state: "Error", message: "Email or password is incorrect" }, { status: 400 });

        const passwordCheck = await verifyPassword(password, user.password);
        if (!passwordCheck) return Response.json({ state: "Error", message: "Email or password is incorrect" }, { status: 400 });
        ...
        console.error(err);
        return Response.json({ state: "Error", message: `Unexpected server error` }, { status: 500 });
    }
}
